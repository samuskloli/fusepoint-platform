...
router.put('/users/:userId/status',
  ...
  async (req, res) => {
    try {
      ...
      console.log('🔍 Debug updateUserStatus - Paramètres reçus:', { userId, isActive, updatedBy });

      if (typeof isActive !== 'boolean') {
        console.error('❌ Erreur: isActive doit être un booléen, reçu:', typeof isActive, isActive);
        return res.status(400).json({
          success: false,
      ...
      if (!user) {
        console.error('❌ Erreur: Utilisateur non trouvé:', userId);
        return res.status(404).json({
          success: false,
      ...
      console.log('✅ Utilisateur trouvé, mise à jour du statut...');
