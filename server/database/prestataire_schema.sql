...
CREATE TABLE IF NOT EXISTS prestataire_invitations (
    ...
    created_user_id INT, -- ID de l'utilisateur créé après acceptation
...
CREATE TABLE IF NOT EXISTS prestataire_tasks (
    ...
    prestataire_id INT NOT NULL,
    agent_id INT NOT NULL,
    client_id INT, -- Optionnel, si la tâche est liée à un client spécifique
...
INSERT IGNORE INTO prestataire_invitations (email, token, agent_id, expires_at, invitation_data) VALUES 
('prestataire.test@example.com', 'test-token-123', 3, DATE_ADD(NOW(), INTERVAL 7 DAY), '{"firstName": "Jean", "lastName": "Dupont", "speciality": "Développement web"}');

-- Exemple de tâche pour test
INSERT IGNORE INTO prestataire_tasks (title, description, prestataire_id, agent_id, task_data) VALUES 
