  ...
  <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click="closeModal">
    <div class="relative top-20 mx-auto p-5 border w-11/12 md:w-2/3 lg:w-1/2 shadow-lg rounded-md bg-white" @click.stop>
      <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-medium text-gray-900">Modifier l'événement</h3>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    onMounted(() => {
      ...
      // S'assurer qu'il y a au moins un champ participant
    ...
    const updateEvent = async () => {
      ...
      try {
        ...
        const response = await projectManagementService.updateEvent(props.event.id, eventData)
        if (response.success) {
