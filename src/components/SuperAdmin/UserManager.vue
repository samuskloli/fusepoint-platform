      ...
      <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
          ...
          <div class="flex justify-end space-x-3">
            ...
            >
              <font-awesome-icon :icon="selectedUser?.is_active ? 'user-slash' : 'user-check'" class="w-4 h-4 mr-2" />
  ...
  methods: {
    ...
    async handleUpdateStatus() {
      try {
        ...
        await superAdminAPI.updateUserStatus(this.selectedUser.id, newStatus);
        ...
        this.$toast.success(`Utilisateur ${newStatus ? 'activé' : 'désactivé'} avec succès`);
      } catch (error) {
