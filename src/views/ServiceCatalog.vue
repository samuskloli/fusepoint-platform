  ...
  setup() {
    ...
    const loadServices = async () => {
      try {
        ...
        const response = await fetch('/api/accompagnement/services', {
          headers: {
    ...
    const handleServiceRequest = async (requestData) => {
      try {
        const response = await fetch('/api/accompagnement/requests', {
          method: 'POST',
          headers: {
    ...
    const formatPrice = (price, priceType) => {
      ...
      if (!price) return `À partir de 0${this.$getCurrencySymbol()}`
