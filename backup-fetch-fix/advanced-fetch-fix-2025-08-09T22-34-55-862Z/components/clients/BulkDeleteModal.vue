    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <p class="text-sm text-gray-500 text-center mb-6">
          ...
          {{ selectedCount > 1 ? messages.clientsPlural : messages.clientsSingular }}?
        </p>
        
        <!-- Liste des clients sélectionnés (optionnel, pour les petites sélections) -->
        <div v-if="selectedClients.length <= 5" class="mb-4">
          <div class="bg-gray-50 rounded-md p-3">
            <p class="text-xs font-medium text-gray-700 mb-2">{{ messages.selectedClientsLabel }}:</p>
            <ul class="text-sm text-gray-600 space-y-1">
              <li v-for="client in selectedClients" :key="client.id" class="flex items-center">
                <span class="w-2 h-2 bg-red-400 rounded-full mr-2"></span>
                {{ client.first_name }} {{ client.last_name }}
                <span v-if="client.email" class="text-gray-400 ml-1">({{ client.email }})</span>
              </li>
            </ul>
          </div>
        </div>
        
        <!-- Avertissement pour les grandes sélections -->
        <div v-else-if="selectedCount > 10" class="mb-4">
          <div class="bg-yellow-50 border border-yellow-200 rounded-md p-3">
            <div class="flex">
              <ExclamationTriangleIcon class="h-5 w-5 text-yellow-400 mr-2 flex-shrink-0" />
              <p class="text-sm text-yellow-800">
                {{ messages.warningLargeSelection }}
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="mb-6">
          <label for="bulk-password" class="block text-sm font-medium text-gray-700 mb-2">
