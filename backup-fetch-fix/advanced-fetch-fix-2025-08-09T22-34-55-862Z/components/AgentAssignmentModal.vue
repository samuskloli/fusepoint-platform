    ...
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white">
      <div class="mt-3">
        ...
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-medium text-gray-900">{{ $t('agentAssignment.assignAgent') }}</h3>
        ...
        <!-- Liste des agents -->
        <div class="mb-4">
          <h4 class="text-sm font-medium text-gray-700 mb-3">{{ $t('agentAssignment.availableAgents') }}</h4>
          
          <!-- État de chargement -->
          <div v-if="loading" class="flex items-center justify-center py-8">
            <div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600"></div>
            <span class="ml-2 text-gray-600">{{ $t('agentAssignment.loadingAgents') }}</span>
          </div>
          ...
          <div v-else class="space-y-3 max-h-96 overflow-y-auto">
            <AgentCard
              ...
              :agent="agent"
              :disabled="assigning"
              :action-label="assigning ? $t('agentAssignment.assigning') : $t('agentAssignment.assign')"
              :show-stats="true"
            ...
            >
              <template #actions="{ agent }">
                ...
                >
                  ...
                  <span>{{ assigning ? $t('agentAssignment.assigning') : $t('agentAssignment.assign') }}</span>
                </button>
              </template>
            </AgentCard>
          </div>
